...
char *create_battery_temperature(float *temperature_value, int size)
{
    ...
    cJSON *name = cJSON_CreateString("Cell battery Temperature");
    if (name == NULL)
        goto end;
...
char *create_battery_current(float *current_value, int size)
{
    ...
    cJSON *name = cJSON_CreateString("Cell battery current");
    if (name == NULL)
        goto end;
                           ...
                           int size)
{
    ...
    cJSON *name = cJSON_CreateString("Battery Cells");
    if (name == NULL)
        goto end;
...
char *create_pheripheral_infor(bool *status_pheripheral)
{
    ...
    if (pheripheral_table == NULL)
        goto end;
    cJSON_AddItemToObject(pheripheral_table, "packages information", pheripheral_values);
...
char *create_voltage_warning(bool* status_voltage,int size){
    ...
    cJSON *name = cJSON_CreateString("Cell status voltage battery");
    if (name == NULL)
        goto end;
    ...
    if (statuses == NULL)
        goto end;
    cJSON_AddItemToObject(battery_voltage_status, "status voltages", statuses);
    for (int i = 0; i < size; i++)
    {
        char battery_template[] = "Battery ";
        char number[10] = {0};
...
char *create_temperature_warning(bool* status_temperature,int size){
    ...
    cJSON *name = cJSON_CreateString("Cell status temperature battery");
    if (name == NULL)
        goto end;
    ...
    if (statuses == NULL)
        goto end;
    cJSON_AddItemToObject(battery_temperature_status, "status temperatures", statuses);
    for (int i = 0; i < size; i++)
    {
        char battery_template[] = "Battery ";
        char number[10] = {0};
